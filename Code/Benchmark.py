...
class Benchmark(object):
    def __init__(self, path):
        #initializing:
        self.receptor_name = path[path.rfind("/")+1:path.rfind("/")+5] # the foleder's name is XXXX or XXXX_n where XXXX is the pdb code
        print '#'*60
    ...
    def _Create_DB2_file(self):
        ...
        os.chdir(self.path)
        sp.call("/work/londonlab/git_dock/DOCK/ligand/generate/build_smiles_ligand.sh xtal-lig.smi --covalent", shell=True)
        os.chdir(self.current_location)
        
    def _Prepare_Receptor(self):
        """prepare receptor for docking"""
        os.chdir(self.path)
        sp.call("/work/londonlab/git_dock/DOCK/proteins/blastermaster/blastermaster.py --covalentResNum {0} --covalentResName {1} --covalentResAtoms {2}".format(self.covalent_res.id[1], self.covalent_res.resname, self.covalent_atom_dict[self.covalent_res.resname]), shell=True) 
        os.chdir(self.current_location)    
