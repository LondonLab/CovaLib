...
def main(name, argv):
	...
	for res in residues:
		...
		if run_mode == '0':
                        state_file = 'rec.pdb'
                        rot_folder = folder + 'Rot0/'
                        dir_f.write(rot_folder + '\n')
                        PyUtils.create_folder(rot_folder)
                        shutil.copy(state_file, rot_folder + 'rot.pdb')
		...
		for rot in os.listdir('./'):
			...
			if run_mode == '1':
				#Only prepare
				if not tart_bool:
					commands.append(' '.join(['python', Paths.SCRIPTS + 'Prepare.py', 'rot.pdb', 'xtal-lig.pdb', 'CYS', res, 'HG']))
			...
			if run_mode == '3':
				#Normal Combine
				commands.append(' '.join(['python', Paths.SCRIPTS + 'Combine.py', argv[5], 'True']))
				#Special getposes
				#os.chdir(argv[5])
