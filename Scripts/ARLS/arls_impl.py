...
def find_programs(options):
    ...
    scr_paths = [os.path.join(options.mini,'src','apps','public','ligand_docking'),
        os.path.join(options.mini,'src','python','apps','public'), os.path.join(options.mini,'scripts','python','public')] + PATH_paths
...
def write_dock_flags(outfile, options, docking_cases): #{{{
    nstruct = 250 // get_procs_per_case(docking_cases, options)
    outfile.write('''
-in
 -file
  ## If you weren't supplying the input file(s) on the command line,
...
setup_docking_case()
{
    ...
    for((i=0;i<$3;i++)); do
        mkdir -p "work/$pl/$i"
        echo "${Executable} @COMMON.flags @DOCK.flags -in:file:s input/$pl.pdb -in:file:native native/$pl.pdb -packing:unboundrot unbound/$p.pdb -out:path:pdb work/$pl/$i -run:seed_offset $i -out:suffix _$i -nodelay > log/out.${pl}.${i}.log 2> log/err.${pl}.${i}.log" >> "$dockfile"
    done
    echo >> "$dockfile"
}
...
''' % progs)
        for d in docking_cases:
            ...
            outfile.write('grep \'SCORES %s\' pdbs/default.out | awk \'{print $106}\' > pdbs/rmsd.txt\n' % prot_lig)
#}}}
