...
#
# If you use HBonanza in your work, please cite Durrant, J. D.; McCammon, J. A., HBonanza: A Computer
# Algorithm for Molecular-Dynamics-Trajectory Hydrogen-Bond Analysis. Journal of Molecular Graphics 
# and Modelling 2011, (doi:10.1016/j.jmgm.2011.07.008).
...
def program_info(): # still need to update this
    ...
    program_output = program_output + "# HBonanza Version 1.01\n"
    program_output = program_output + "# =================\n\n"
    program_output = program_output + "# If you use HBonanza in your research, please cite the following reference:\n"
    program_output = program_output + "# Durrant, J. D.; McCammon, J. A., HBonanza: A Computer Algorithm for\n"
    program_output = program_output + "# Molecular-Dynamics-Trajectory Hydrogen-Bond Analysis. Journal of Molecular\n"
    program_output = program_output + "# Graphics and Modelling 2011, (doi:10.1016/j.jmgm.2011.07.008).\n\n"
    ...
    def id_hydrogen_bonds(self, arguments):
        ...
        if self.N_O_F_S is None: # so not predefined
            for index in self.AllAtoms:
                ...
                if atom.element == "O" or atom.element == "N" or atom.element == "F" or atom.element == "S":
                    if atom.resid in arguments['SEED_RESIDUES']: #CHANGE-DANIEL
        ...
        for index1 in range(len(N_O_F_S)-1):
            ...
            for index2 in range(index1 + 1, len(N_O_F_S)):
                ...
                main_seed = arguments['SEED_RESIDUES'][0]
                if (atom1.resid == main_seed and not atom2.resid == main_seed) or (atom2.resid == main_seed and not atom1.resid == main_seed): #CHANGE-DANIEL
                    dist = atom1.coordinates.dist_to(atom2.coordinates)
...
def residues_connected_to_the_seeds(residue_connections):
    ...
    if len(arguments['SEED_RESIDUES']) > 0: # so a seed as been specified
        ...
        return connected_residues_growing_list #CHANGE-DANIEL
        if arguments['JUST_IMMEDIATE_CONNECTIONS'] == "FALSE": # so you're going to "recursively" walk through all neighbors
            ...
            tmp = []
            print residue_connections
            print connected_residues_growing_list
            for theresid in connected_residues_growing_list:
                if theresid in residue_connections: #CHANGE-DANIEL
                    for connecteds in residue_connections[theresid]:
...
def calculate_average_frequency_relevant_atoms_participate_in_hydrogen_bonds(single_pdb, atom_vals):
    global arguments
    f_hb = open(arguments['OUTPUT_BASENAME'] + 'HBonds', 'w') #CHANGE-DANIEL
    atom_average_vals = {}
    extra_file_string = ""
    for key in atom_vals.keys():
        atom_average_vals[key] = average(atom_vals[key])
        tmp = single_pdb.AllAtoms[key]
        #if not arguments['SEED_RESIDUES'][0] == tmp.resid:
        f_hb.write(str(tmp.atomname) + ' ' + str(tmp.resid) + '\n')
        if arguments['WRITE_COLUMN'] == "O":
...
if len(atom_vals) == 0: print "# WARNING: No hydrogen bonds detected. Does this model include hydrogen bonds?"
