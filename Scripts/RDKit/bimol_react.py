...
def main(name, argv):
    ...
    for i, rxn in enumerate(rxns):
        ...
        has_patt_2 = list(map(lambda m: m[0].HasSubstructMatch(reactents_smarts[1]), molecules))
        reactent_option_2 = [m for i,m in enumerate(molecules) if has_patt_2[i]]
    ...
    with open(argv[2], 'w') as file:
        for mol1 in reactent_option_1:
            for mol2 in reactent_option_2:
                ...
                #delete if you want all options
                if len(products) > 1:
                    continue
                # write to file
