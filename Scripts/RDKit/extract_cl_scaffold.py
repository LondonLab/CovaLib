...
def main(name, argv):
	...
	rxn = rdChemReactions.ReactionFromSmarts('[N:2][C:3](=[O:4])[C:1]Cl>>[N:2][C:3](=[O:4])[C:1]n1nnnn1')
	reactents_smarts = rxn.GetReactants()
	back = rdChemReactions.ReactionFromSmarts('[C:1]n1nnnn1>>[C:1]Cl')
	...
	with open(argv[1], 'w') as f:
		for line in smiles:
			...
			if core.HasSubstructMatch(back_smarts[0]):
				...
				scaffold = Chem.MolFromSmiles('O=C(N)CCl')
